...
returns trigger language plpgsql security definer set search_path=public as $$
declare rid uuid; act text; sm text;
begin
  rid := coalesce(new.id,old.id);
  act := case when tg_op='INSERT' then '생성' when tg_op='UPDATE' then '수정' else '삭제' end;
  sm := case tg_table_name
    when 'project' then coalesce(new.project_name,old.project_name)
    when 'client' then coalesce(new.name,old.name)
    when 'quote' then coalesce(new.quote_no,old.quote_no)
    when 'billing' then coalesce(new.billing_type,old.billing_type)
    when 'task' then coalesce(new.title,old.title)
    when 'staff' then coalesce(new.name,old.name)
