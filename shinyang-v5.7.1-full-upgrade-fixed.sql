...
create policy service_contract_read on public.service_contract for select using (
 ...
 (app_current_role()='STAFF' and (project_id is null or exists(select 1 from public.project p where p.id=project_id and p.manager_id=auth.uid())))
);
...
-- v5.7.5 contract compatibility
alter table public.service_contract add column if not exists customer_representative text;
alter table public.service_contract add column if not exists customer_contact_name text;
alter table public.service_contract add column if not exists customer_contact_phone text;
