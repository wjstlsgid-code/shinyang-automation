...
export async function POST(req: Request) {
  ...
  if (!contractId || !pdfBase64) {
    return NextResponse.json(
      { error: '계약서 정보 또는 PDF가 없습니다.' },
      { status: 400 }
    )
  }

  const { data: c, error } = await auth.userClient
  ...
  if (
    ...
    c.project?.manager_id &&
    c.project.manager_id !== auth.user.id
  ) {
    return NextResponse.json(
      { error: '본인 담당 프로젝트 계약서만 발송할 수 있습니다.' },
      { status: 403 }
    )
  }

  const to = c.client?.email
  ...
  const contractMailTitle = String(
    c.service_name || '환경컨설팅'
  )
    .replace(/\s*용역\s*$/, '')
    .trim()

  const subject =
    `[신양파트너스] ${contractMailTitle} 용역계약서 송부`

  const text =
`${c.client?.name || ''} 담당자님, 안녕하세요.
신양파트너스입니다.
...
감사합니다.`
  ...
  const filename =
    `${c.contract_no}_${String(c.client?.name || '거래처')
      .replace(/[\\/:*?"<>|]/g, '_')}_용역계약서.pdf`

  try {
    const sent = await sendHiworksMail({
      to,
      subject,
      text: `${text}${buildSignatureText(
        senderLine,
        senderMobile
      )}`,
      html,
    ...
    await auth.userClient
      ...
      .insert({
        ...
        entity_id: c.id,
        action: 'EMAIL_SENT',
        summary: `${c.contract_no} 계약서 발송 · ${to}`,
      })
